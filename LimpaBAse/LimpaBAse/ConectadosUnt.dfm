object ConectadosFrm: TConectadosFrm
  Left = 0
  Top = 0
  BorderIcons = []
  BorderStyle = bsSingle
  Caption = 'Usu'#225'rios Conectados'
  ClientHeight = 273
  ClientWidth = 492
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poOwnerFormCenter
  PixelsPerInch = 96
  TextHeight = 13
  object CancelarImg: TImage
    Left = 462
    Top = 243
    Width = 22
    Height = 22
    Hint = 'Cancelar'
    ParentShowHint = False
    Picture.Data = {
      
      00000049454E44AE426082}
    ShowHint = True
    OnClick = CancelarImgClick
  end
  object OkImg: TImage
    Left = 434
    Top = 243
    Width = 22
    Height = 22
    Hint = 'Confirmar'
    ParentShowHint = False
    Picture.Data = {
      
      454E44AE426082}
    ShowHint = True
    OnClick = OkImgClick
  end
  object ConectadosGrd: TDBGrid
    Left = 8
    Top = 8
    Width = 476
    Height = 229
    DataSource = ConectadosDtSrc
    ReadOnly = True
    TabOrder = 0
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'Tahoma'
    TitleFont.Style = []
    Columns = <
      item
        Expanded = False
        FieldName = 'TERMINAL'
        Width = 220
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'PROGRAM'
        Title.Caption = 'PROGRAMA'
        Width = 220
        Visible = True
      end>
  end
  object Panel: TPanel
    Left = 8
    Top = 243
    Width = 420
    Height = 22
    Caption = 'Verifique os usu'#225'rios conectados antes de continuar!'
    Color = clRed
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWhite
    Font.Height = -16
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentBackground = False
    ParentFont = False
    TabOrder = 1
  end
  object ConectadosDtSrc: TDataSource
    DataSet = BancoDadosDtMdl.ConectadosQry
    Left = 216
    Top = 96
  end
end
