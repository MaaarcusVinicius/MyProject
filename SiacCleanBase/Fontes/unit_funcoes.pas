...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if ((Form.Components[i] as TEdit).Hint <> '') and (TrocaCaracterEspecial((Form.Components[i] as TEdit).Text, true) = '') then
        begin
      ...
      begin
        if ((Form.Components[i] as TMaskEdit).Hint <> '') and ((Form.Components[i] as TMaskEdit).Text = '') then
        begin
      ...
      begin
        if ((Form.Components[i] as TComboBox).Hint <> '') and ((Form.Components[i] as TComboBox).Text = '') then
        begin
      ...
      begin
        if ((Form.Components[i] as TDBLookupComboBox).Hint <> '') and ((Form.Components[i] as TDBLookupComboBox).Text = '') then
        begin
...
const
  ...
  xCarExt: array[1..48] of string = ('<', '>', '!', '@', '#', '$', '%', '¨', '&', '*', '(', ')', '_', '+', '=', '{', '}', '[', ']', '?', ';', ':', ',', '|', '*', '"', '~', '^', '´', '`', '¨', 'æ', 'Æ', 'ø', '£', 'Ø', 'ƒ', 'ª', 'º', '¿', '®', '½', '¼', 'ß', 'µ', 'þ', 'ý', 'Ý');
var
  xTexto: string;
  i: Integer;
begin
